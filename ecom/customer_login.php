<?php 
	...
	if($_SERVER['REQUEST_METHOD']=='POST')
	{
		...
		{
			$query="SELECT * from users_table where username='".$u."' and user_type_id='1'";
			...
			if ($result->num_rows > 0) {
				while($row = $result->fetch_assoc())
				{
					if($row["password"]==md5($p))
					{
						...
						echo "<script>alert('Login Success!');window.location=\"./customer_home.php\";</script>";
					}
					else{
						echo "<script>alert('Wrong credentials provided!');window.location=\"./customer_login.php\";</script>";
					}
				}
			}
			else{
